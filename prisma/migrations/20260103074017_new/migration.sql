...
CREATE TABLE `schools` (
    ...
    `subscriptionTier` VARCHAR(191) NOT NULL DEFAULT 'trial',
    ...
    UNIQUE INDEX `schools_stripeCustomerId_key`(`stripeCustomerId`),
    PRIMARY KEY (`id`)
...
CREATE TABLE `subscription_plans` (
    ...
    INDEX `subscription_plans_isPublic_idx`(`isPublic`),
...
CREATE TABLE `plan_change_history` (
    ...
    INDEX `plan_change_history_schoolId_idx`(`schoolId`),
    INDEX `plan_change_history_oldPlanId_idx`(`oldPlanId`),
    INDEX `plan_change_history_newPlanId_idx`(`newPlanId`),
    INDEX `plan_change_history_changedBy_idx`(`changedBy`),
    PRIMARY KEY (`id`)
) DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
-- AddForeignKey
ALTER TABLE `plan_change_history` ADD CONSTRAINT `plan_change_history_schoolId_fkey` FOREIGN KEY (`schoolId`) REFERENCES `schools`(`id`) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `plan_change_history` ADD CONSTRAINT `plan_change_history_oldPlanId_fkey` FOREIGN KEY (`oldPlanId`) REFERENCES `subscription_plans`(`id`) ON DELETE SET NULL ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE `plan_change_history` ADD CONSTRAINT `plan_change_history_newPlanId_fkey` FOREIGN KEY (`newPlanId`) REFERENCES `subscription_plans`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
