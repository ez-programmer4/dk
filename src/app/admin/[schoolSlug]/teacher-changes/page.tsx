...
export default function TeacherChangeManagement() {
  ...
  const loadData = async () => {
    try {
      ...
      const [changesRes, teachersRes, studentsRes] = await Promise.all([
        fetch(`/api/admin/${schoolSlug}/teacher-changes`),
        fetch(`/api/admin/${schoolSlug}/users?role=teacher`),
  ...
  const handleAddChange = async () => {
    try {
      const response = await fetch(`/api/admin/${schoolSlug}/teacher-changes`, {
        method: "POST",
