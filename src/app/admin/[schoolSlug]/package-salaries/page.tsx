...
export default function PackageSalariesPage() {
  ...
  const fetchSalaries = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/${schoolSlug}/package-salaries`);
      if (response.ok) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const url = isEditing
        ? `/api/admin/${schoolSlug}/package-salaries/${editingSalary.salaryId}`
        : `/api/admin/${schoolSlug}/package-salaries`;
  ...
  const handleDelete = async (id: number) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`/api/admin/${schoolSlug}/package-salaries/${id}`, {
        method: "DELETE",
      });
