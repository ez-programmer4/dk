...
export default function DeductionAdjustmentsPage({
  ...
  const fetchTeachers = async () => {
    try {
      const res = await fetch(`/api/admin/${schoolSlug}/teachers`);
      if (res.ok) {
  ...
  const fetchTimeSlots = async () => {
    try {
      const res = await fetch(`/api/admin/${schoolSlug}/time-slots`);
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchWaiverHistory = async () => {
    try {
      const res = await fetch(
        `/api/admin/${schoolSlug}/deduction-adjustments/history?limit=50`
      );
      if (res.ok) {
        const data = await res.json();
  ...
  const previewAdjustments = async (studentIds?: string[]) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/admin/${schoolSlug}/deduction-adjustments/preview`, {
        method: "POST",
  ...
  const handleAdjustment = async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/admin/${schoolSlug}/deduction-adjustments`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          adjustmentType,
      ...
      if (res.ok) {
        ...
        const appliedAdjustment = {
          ...
          totalAmountWaived:
            data.financialImpact?.totalDisplayAmount ||
