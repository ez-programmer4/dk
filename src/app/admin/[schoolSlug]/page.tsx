  ...
  const fetchDashboardWidgets = useCallback(async () => {
    ...
    try {
      ...
      const [
        ...
        fetch(`/api/admin/${schoolSlug}/permissions?status=Pending`),
        fetch(`/api/admin/${schoolSlug}/payments`),
    ...
    const fetchData = async () => {
      ...
      try {
        const [statsRes, analyticsRes] = await Promise.all([
          fetch(`/api/admin/${schoolSlug}/stats?${params.toString()}`),
          fetch(`/api/admin/${schoolSlug}/analytics?${params.toString()}`),
        ]);
