...
export class SalaryCalculator {
  ...
  /**
   * 🆕 NEW METHOD: Calculate teaching days for a specific daypackage in a month
   * This counts the actual number of days in the month that match the student's daypackage
   *
   * Examples:
   * - MWF: Counts Mondays, Wednesdays, Fridays in the month (typically 12-13 days)
   * - TTS: Counts Tuesdays, Thursdays, Saturdays in the month (typically 12-13 days)
   * - All Days: Counts all days (excluding/including Sunday based on config)
   *
   * @param monthStart - First day of the month
   * @param monthEnd - Last day of the month
   * @param daypackage - Student's daypackage (MWF, TTS, All Days, etc.)
   * @returns Number of teaching days in the month for this daypackage
   */
  private calculateTeachingDaysForDaypackage(
    ...
    if (!daypackage || daypackage.trim() === "") {
      // If no daypackage, use all days (let Sunday setting decide)
      return this.calculateWorkingDays(monthStart, monthEnd);
    }

    // Parse daypackage to get expected days of week
    const expectedDaysOfWeek = this.parseDaypackage(daypackage);

    if (expectedDaysOfWeek.length === 0) {
      // Invalid daypackage, fallback to all working days
      return this.calculateWorkingDays(monthStart, monthEnd);
    }
    ...
    while (current <= end) {
      ...
      // Check if this day matches the daypackage
      if (expectedDaysOfWeek.includes(dayOfWeek)) {
        // For "All Days" package, still respect Sunday inclusion setting
        if (expectedDaysOfWeek.length === 7) {
          // All days package
          const isSunday = dayOfWeek === 0;
          const shouldInclude = this.config.includeSundays || !isSunday;
          if (shouldInclude) {
          ...
          // Specific days package (MWF, TTS) - always count matching days
          teachingDays++;
        }
      }
  ...
  private async calculateBaseSalary(
    ...
    // NOTE: This is now used as a fallback for students without daypackage
    // Per-student daily rates are calculated based on their specific daypackage (see below)
    ...
    for (const student of students) {
      ...
      const studentTeachingDaysInMonth =
        this.calculateTeachingDaysForDaypackage(
          monthStart,
          monthEnd,
          studentDaypackage
        );
      ...
      if (student.teacherChangePeriod) {
        ...
        // 🆕 NEW FIX: Calculate daily rate based on student's specific daypackage teaching days
        // Example: MWF student = ~12-13 days/month, not 30 days
        dailyRate =
          studentTeachingDaysInMonth > 0
            ? Number(
      ...
      // 🆕 Reuse studentDaypackage from earlier in the function (already set at line 1492)
...
Teacher Change Period: ${student.teacherChangePeriod ? "Yes" : "No"}`;
      ...
      for (const period of periods) {
        ...
        periodZoomLinks.forEach((link: any) => {
          if (link.sent_time) {
            ...
            // Check Sunday in UTC
            const isSunday = sentTime.getUTCDay() === 0;
            const shouldInclude = this.config.includeSundays || !isSunday;
  ...
  private async calculateAbsenceDeductions(
    ...
    try {
      ...
      // Helper function to parse daypackage (uses same logic as main parseDaypackage method)
      // This ensures consistency across the codebase
      const parseDaypackage = (dp: string): number[] => {
        return this.parseDaypackage(dp);
      };
